...
git clone --depth=1 https://$USERNAME:$TOKEN@github.com/strongreasons/kernel_asus_sdm660 -b hmp-oc-12 $DEVICE_CODENAME

ClangPath=${MainClangZipPath}
[[ "$(pwd)" != "${MainPath}" ]] && cd "${MainPath}"
mkdir $ClangPath
rm -rf $ClangPath/*
wget -q  https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/refs/heads/master/clang-r450784e.tar.gz -O "clang-r450784e.tar.gz"
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 [Onyx][$KERNELNAME]-HMP-$DEVICE_CODENAME-LV-4.4.205.zip *
    cd ..
}
compile
zipping
